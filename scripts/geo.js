...
function GEOprocess(position) {
	...
	$('#latlng').attr('data-latlng', position.coords.latitude + ',' + position.coords.longitude)
...
function GEOdeclined(error) {
	$('#alert').html('Error: ' + error.message).show();
}

if (navigator.geolocation) {
	navigator.geolocation.getCurrentPosition(GEOprocess, GEOdeclined);
} else {
	$('#alert').html('Your browser sucks. Upgrade it.').show();
}
